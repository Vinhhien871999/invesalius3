...
| C7 | Rebuild mesh sau khi sửa mask (surface update) | `segmentation_panel.py._on_update_surface` → `"Create surface from index"` | ✓ | ✓ | ✓ | ✓ | **WORKING** (Phase 08) | Xem mục D9 — cùng cơ chế, cùng kết luận, cùng bằng chứng Phase 08 |
...
| D9 | Mask sửa → Surface 3D cập nhật | `segmentation_panel.py._on_update_surface` | ✓ | ✓ | ✓ | ✓ | **WORKING** (Phase 08, 14/09/2026) | **Nguyên nhân gốc thật sự tìm ra ở Phase 08** (không phải RAM/timing như nghi ngờ ở vòng 3): `_on_update_surface` luôn hardcode `algorithm="Default"` — đọc trực tiếp `invesalius/data/surface_process.py.create_surface_piece()` xác nhận: với `Default`+`from_binary=False`, marching cubes contour lại ẢNH GỐC theo `threshold_range`, **không hề đọc `mask.matrix`** — mọi chỉnh sửa mask (brush/region growing/undo-redo) vì vậy vô tác dụng. Xác nhận chéo bằng chính native dialog InVesalius (`gui/dialogs.py.SurfaceMethodPanel`): dialog gốc tự ẩn "Default" và cảnh báo *"not possible to use Default because mask was edited"* khi `mask.was_edited==True`, chuyển sang thuật toán đọc mask thật. **Đã sửa**: `_on_update_surface` chọn `algorithm="Binary"` khi `mask.was_edited==True` (giữ `"Default"` cho mask threshold chưa sửa — không regression); `_on_region_grown` bổ sung `was_edited=True`. **Verify runtime thật (dataset tổng hợp nhỏ 30×128×128 để né giới hạn RAM đã gặp ở vòng 3, chạy 100% qua code thật)**: 36/36 check PASS, 3 chu kỳ sửa mask→rebuild liên tiếp, polydata thật đổi đúng mỗi lần (points 7296→13440→16368→18720, cells 14588→26876→32732→37436, bounds giãn đúng hướng), không duplicate surface/actor, `Render()` thành công, 0 crash report mới. **Còn lại**: xác nhận thủ công qua GUI thật (chuột thật) chưa làm — xem `CT3D_P08_ROI3D_CLOSURE_REPORT.md` mục P08.5. Chi tiết đầy đủ: `CT3D_P08_ROI3D_CLOSURE_REPORT.md` |
