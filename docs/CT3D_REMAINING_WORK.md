...
### 1c. [P1, hợp nhất ở Phase 09] Manual QA thao tác chuột thật — 7 mục
...
- **Cách hoàn thiện**: làm theo checklist đầy đủ (9 mục con, có công thức đối chiếu sai số cho E2/E3) trong `CT3D_P09_INTERACTION_QA_REPORT.md` mục 9. Ước lượng: 30-45 phút thao tác tay cho cả 7 mục.
...
> **Cập nhật sau Phase 11 (14/09/2026)**: D9/C7 (Phase 08), Sync 2D→3D và F3 (Phase 09), checksum Save/Open "2a" (Phase 10, CASE A/đóng) đã đóng. Undo/Redo đã đo + tối ưu bộ nhớ, số liệu worst-case đã sửa lại đúng ở Phase 11 (273.6MB, không phải 547MB — xem `CT3D_P11_TEST_AUTOMATION_REPORT.md` mục 11-12). Mục "8" (dead code `MaskEditor.draw_point_2d/draw_point_3d/interpolate_slices` + 7 method liên quan khác phát hiện thêm) **đã xoá thật (146 dòng) ở Phase 11**, không còn là việc-chưa-xong — xoá khỏi bảng này. Toàn bộ bằng chứng quan trọng của Phase 08-10 (từng nằm trong script scratchpad tạm thời) nay đã có bộ `pytest` bền vững thật trong `tests/ct3d/` (109 test). Bảng dưới đây phản ánh đúng baseline SAU Phase 11.
...
| 9 | `MaskEditorManager.set_current_mask/get_current_editor/delete_editor` chưa xác nhận call-site (`NOT_PROVEN_DEAD`, phát hiện Phase 11, chưa điều tra kỹ) | P3 | 15-30 phút grep + xác nhận, tương tự quy trình đã làm với `MaskEditor` ở Phase 11 |
