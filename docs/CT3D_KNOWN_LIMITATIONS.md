...
| 10 | Export NRRD cần dependency tuỳ chọn (`pynrrd`) | `SOFTWARE_LIMITATION` | Không bundle mặc định — UI phát hiện + cảnh báo rõ khi thiếu (`_is_nrrd_available()`), người dùng tự `pip install pynrrd` (hoặc extra `nrrd` trong `pyproject.toml`) nếu cần định dạng này. Không phải bug, là lựa chọn dependency tối giản. |
